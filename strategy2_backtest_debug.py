#!/usr/bin/env python3
"""
威科夫事件 + 高价值VSA标签策略 (Strategy 2) 回测框架 - 简化版本
"""
...
class Strategy2Backtester:
    ...
    def evaluate_strategy_2(self, df, i, wevents, nt, vsa_labels):
        ...
        # print(f"Checking index {i} for Strategy 2")
        # print(f"nt['buy_passed']: {nt['buy_passed']}")
        
        if nt["buy_passed"] >= 7:
            # 检查是否有高价值事件，且事件置信度较高
            high_events = [e for e in wevents if e["type"] in ["Spring", "Shakeout", "SOS", "JOC", "ST"] and e.get("conf", 0) > 80]
            # print(f"High events found: {len(high_events)}")
            ...
            if high_events:
                ...
                # print(f"High VSA labels found: {len(high_vsa)}")
