...
class CodeSearchDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        ti = QLabel("键盘精灵", head)
        ti.setStyleSheet(f"color:#ffffff;font-weight:bold;font-size:{theme.font_pt('body')};"
                         "background:transparent;")
        hh.addWidget(ti)
        tip = QLabel("输入代码/名称/拼音  (Esc 关闭)", head)
        tip.setStyleSheet(f"color:rgba(255,255,255,200);font-size:{theme.font_pt('caption')};"
                          "background:transparent;")
        hh.addWidget(tip)
        hh.addStretch(1)
        root.addWidget(head)
        ...
        self.entry.setStyleSheet(
            ...
            f"padding:5px 8px;font-size:{theme.font_pt('body')};")
        bl.addWidget(self.entry)

        self.result_list = QListWidget(body)
        self.result_list.setStyleSheet(
            f"background:{theme.C_BG};color:{theme.C_TEXT};"
            f"border:1px solid {theme.C_BORDER};border-radius:4px;font-size:{theme.font_pt('body')};")
