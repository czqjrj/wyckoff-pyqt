...
class OptimizedStrategy1:
    """优化版策略1：多因子强化做强多头策略"""
    
    def __init__(self, data_dir="optimized_strategy_data"):
    ...
    def evaluate_optimized_strategy_1(self, df, i, wevents, nt, vsa_labels, stock_code):
        """优化策略1：多因子强化做强多头策略"""
        # 只做强多头事件
        ...
        if bull_events:
            ...
            if (market_conditions["ma20_upward"] and 
                market_conditions["sector_strength"] > 0.6 and 
                market_conditions["fund_flow"] > 0.5):
                
                return {
                    "strategy": "optimized_strategy_1",
                    "signal": "multi_factor_bull",
                    "confidence": 95,
                    "details": f"强多头事件: {event['type']} (conf≥90) + 市场条件满足",
                    "event": event,
                    "market_conditions": market_conditions,
                    "requirements_met": "所有硬门槛条件"
                }
        return None
    ...
    def analyze_stock(self, code, datalen=1000, horizon=20, cost=0.004):
        ...
        for i in sample_points:
            ...
            # 应用优化策略1
            strategy1_result = self.evaluate_optimized_strategy_1(df, i, wevents, nt, vsa_labels, code)
            if strategy1_result:
...
def main():
    ...
    # 创建优化策略管理器
    optimizer = OptimizedStrategy1()
    
    # 加载历史数据
    optimizer.load_performance_history()
    ...
    for stock in test_stocks:
        try:
            result = optimizer.analyze_stock(stock, datalen=1000)
    ...
    for result in analysis_results:
        if "error" not in result:
            ...
            print(f"{result['stock']} ({result['name']}): 发现 {count} 个优化信号")
            
            # 显示具体信号
            for signal in result["strategies_found"]:
                print(f"  - {signal['details']}")
                print(f"    置信度: {signal['confidence']}%")
                print(f"    市场条件: {signal['market_conditions']}")
                print()
