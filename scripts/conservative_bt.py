...
def _load_dedup_args(dedup, sector_map_path):
    """构造 (dedup, sector_map, chain_map)。sector/chain 无映射时模块自动退化为 date。

    chain 模式若未单独提供链条映射, 由 code→板块映射经 chain_map_from_sectors 派生。
    """
    sector_map = None
    if sector_map_path:
    ...
    chain_map = None
    if dedup == "chain":
        chain_map = cbt.chain_map_from_sectors(sector_map)
    return dedup, sector_map, chain_map
...
def main():
    ...
    ap.add_argument("--dedup", choices=("none", "date", "sector", "chain"),
                    default="none", help="去重模式: none/date/sector/chain(同产业链同日去重)")
    ...
    dedup, sector_map, chain_map = _load_dedup_args(args.dedup, args.sector_map)
