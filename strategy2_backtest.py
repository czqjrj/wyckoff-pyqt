#!/usr/bin/env python3
"""
威科夫事件 + 高价值VSA标签策略 (Strategy 2) 回测框架
使用1年A股历史数据进行回测分析
"""
...
class Strategy2Backtester:
    ...
    def backtest_strategy_2(self, symbol, datalen=250, horizon=20, cost=0.004):
        ...
        for i in sample_points:
            ...
            if strategy2_result:
                # 只保留高置信度信号
                if strategy2_result["confidence"] >= 85:
                    signals.append({
                        "timestamp": df.iloc[i]["day"],
                        "index": i,
        ...
        for signal in signals:
            ...
            if signal_idx + horizon < len(df):
                ...
                trades.append({
                    ...
                    "exit_date": df.iloc[exit_idx]["day"],
        ...
        if trades:
            ...
            max_drawdown = 0  # 简化处理，实际应该计算最大回撤
            sharpe_ratio = 0  # 简化处理
    ...
    def generate_report(self, stocks_list, datalen=250, horizon=20):
        ...
        report = {
            ...
            "total_trades": total_trades,
            "strategy": "威科夫事件 + 高价值VSA标签 (Strategy 2)",
    ...
    def plot_performance(self, report):
        """绘制性能图表"""
        try:
            # 准备数据
            trades = []
            for stock, result in report["stock_results"].items():
            ...
            plt.pie([win_rate, lose_rate], labels=['盈利', '亏损'], autopct='%1.1f%%')
            plt.title('交易胜率')
            ...
            bars = plt.bar(['平均盈利', '平均亏损'], [avg_win, avg_loss], color=['green', 'red'])
            plt.title('平均收益对比')
            plt.ylabel('收益率 (%)')
            for bar in bars:
            ...
            # 5. 收益率箱线图
            plt.subplot(2, 3, 5)
            plt.boxplot(returns, patch_artist=True)
            plt.title('收益率箱线图')
            plt.ylabel('收益率')
            ...
            for stock, result in report["stock_results"].items():
                if "total_trades" in result:
                    stock_counts.append(result["total_trades"])
                    stock_names.append(stock)
            
            bars = plt.bar(range(len(stock_counts)), stock_counts)
            plt.title('各股票交易数量')
            plt.xlabel('股票')
            plt.ylabel('交易数量')
            ...
            plt.tight_layout()
            plt.savefig(f"{self.data_dir}/strategy2_backtest_performance.png", dpi=300, bbox_inches='tight')
            plt.close()
            
            print("性能图表已保存至:", f"{self.data_dir}/strategy2_backtest_performance.png")
...
def main():
    ...
    print(f"平均单笔收益: {metrics['avg_return_per_trade']:.2f}%")
    ...
    # 绘制性能图表
    backtester.plot_performance(report)
