...
# bundle schema 版本: 字段结构变更时递增, 低版本端遇到高版本应拒绝导入
SCHEMA_VERSION = 1
...
def merge_feedback(local, remote):
    ...
    for r in remote or []:
        ...
        if rr > cr or (rr == cr and rr == 1 and _fb_time(r) > _fb_time(cur)):
            merged[k] = dict(r)
            n_upd += 1
    return [merged[k] for k in order], n_new, n_upd
...
def model_trained_ts(state):
    ...
    return _num(state.get("trained_at")) if state.get("trained_at") else _num(state.get("trained_ts"))
...
def merge_model(local_state, remote_state, feat_version=None):
    ...
    if fv is None:
        ...
        fv = FEATURE_VERSION
    try:
        remote_fv = int(remote_state.get("feat_version", 0) or 0)
    except (TypeError, ValueError):
        remote_fv = -1
    if remote_fv != fv:
        return None, "feat_version_mismatch"
    if isinstance(local_state, dict) and local_state:
        if model_trained_ts(remote_state) <= model_trained_ts(local_state):
