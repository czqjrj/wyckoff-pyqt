#!/usr/bin/env python3
"""
威科夫事件 + 高价值VSA标签策略 (Strategy 2) 回测框架 - 优化版
"""
...
class Strategy2Backtester:
    ...
    def evaluate_strategy_2(self, df, i, wevents, nt, vsa_labels):
        """策略2: 威科夫事件 + 高价值VSA标签"""
        # Relaxed condition: use >= 6 instead of >= 7 to increase signal frequency
        if nt["buy_passed"] >= 6:
            # 检查是否有高价值事件，且事件置信度较高
    ...
    def backtest_strategy_2(self, symbol, datalen=250, horizon=20, cost=0.004):
        ...
        for i in sample_points:
            ...
            if strategy2_result:
                # 只保留高置信度信号
                if strategy2_result["confidence"] >= 80:  # Lower threshold to catch more signals
                    signals.append({
