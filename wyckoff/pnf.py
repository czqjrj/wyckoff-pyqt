...
# 低概率端收缩折扣 (按档位): 预测 p<_DISCOUNT_HI 时对目标到达概率施加收缩,
# 校正「空间大/因果弱时高估」 (见 _pnf_targets_at._prob 内注释的实证)。
# 保守档 (最近目标) 惩罚最重, 激进档最轻。
# 2026-09-19 重标定 (608 段 = 合成530 + 真实78): 5.4 的全档折扣把原始
# 0.60~0.70 概率整体压进 50~60% 带, 造成 −14~−19pt 低估 (方向桶 −3~−8)。
# 系数整体上调 (保守 0.70→0.84 / 中 0.85→0.94 / 激进 0.94→0.96) 并把过渡窗
# 提前: 全档折扣 <0.55, 0.55~0.72 线性回 1.0。重标定后方向=上/下桶校准差
# 收敛到 ±2.7pt, 50~60% 带 max 6pt, <50% 带 +5~6pt (仍 <±10pt 容忍)。
# 已知残余: 60~70%/70~80% 保守档仍偏低 ~6pt, 属 s_base 空间衰减曲线层问题。
_LOW_DISCOUNT = {"保守": 0.84, "中": 0.94, "激进": 0.96}
_DISCOUNT_LO = 0.55   # 全档折扣的预测概率上限
_DISCOUNT_HI = 0.72   # 过渡窗终点: ≥此值不收缩


def _low_prob_discount(p: float, tier_key: str) -> float:
    """低概率端收缩系数 (1.0 = 不收缩)。

    实证 (2026-09-19 重标定, 608 段): 5.4 折扣过度把 50~60% 带打成 −14~−19pt
    低估 (实际到达率 67~74% vs 预测 54%); 系数上调 + 过渡窗提前后方向桶
    校准差收敛到 ±2.7pt。低端 (<50%) 略回升到 +5~6pt 偏高, 仍在容忍内。
    单调连续: p<_DISCOUNT_LO 取全档折扣, p=_DISCOUNT_HI 处系数回到 1.0。
    """
    if p < _DISCOUNT_LO:
        return float(_LOW_DISCOUNT[tier_key])
    if p < _DISCOUNT_HI:
        w = (_DISCOUNT_HI - p) / (_DISCOUNT_HI - _DISCOUNT_LO)
