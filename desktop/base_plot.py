...
交互约定 (不改 pyqtgraph 库源码, 全部在控件层重写实现):
  - 滚轮      以光标位置为锚点缩放, 缩放后光标下的数据点保持在光标下方;
              悬停哪个面板就缩放哪个面板 (未注册量程的面板回退到主面板)
  - 键盘      上箭头 / + 放大, 下箭头 / - 缩小 (以视图中心为锚点);
              左右箭头平移; Home/R 复位全幅; Backspace/F 视图历史前进后退
  - 左键拖拽  平移 (保留 ViewBox 默认行为), 范围由 ViewBox.setLimits 约束
  - 双击      复位到全幅
  - 十字光标  通过 attach_crosshair 挂载 desktop.crosshair.Crosshair
...
class BasePlotWidget(pg.GraphicsLayoutWidget):
    """多面板 pyqtgraph 图表基类。
    ...
    # ── 核心几何 ──
    def zoom_about(self, ax, factor, plot=None, ay=None):
        ...
        if lim:
            ...
            self.apply_view(nx0, nx1, plot=plot)
        else:
            new_span = max(span * factor, self.MIN_SPAN_X)
            t = min(max((ax - x0) / span, 0.0), 1.0)
            nx0 = ax - new_span * t
            self.apply_view(nx0, nx0 + new_span, plot=plot)
    ...
    def zoom_center(self, factor):
        ...
        vb = plot.getViewBox()
        xr, yr = vb.viewRange()
        ay = (yr[0] + yr[1]) / 2.0 if self._yzoom.get(plot) else None
        self.zoom_about((xr[0] + xr[1]) / 2.0, factor, plot=plot, ay=ay)

    def apply_view(self, x0, x1, push=True, plot=None):
        ...
        if lim:
            ...
            return None
        plot.getViewBox().setXRange(float(x0), float(x1), padding=0)
        if push:
    ...
    # ── 十字光标 ──
    def attach_crosshair(self, plot, fmt_x, fmt_y, font_size=8):
        """挂载十字光标到指定面板, 返回实例供调用方保存。"""
        ch = Crosshair(plot, fmt_x, fmt_y, font_size=font_size)
    ...
    def wheelEvent(self, ev):
        """滚轮: 光标锚点缩放。事件在控件层消费, 不再下发 ViewBox 内置缩放。"""
    ...
    def keyPressEvent(self, ev):
        """键盘: 上/+ 放大, 下/- 缩小 (中心锚点); 左右平移; Home 复位;
        Backspace/F 视图历史。其余按键交还父类 (保留 ViewBox 行为)。"""
        if not self._has_data:
